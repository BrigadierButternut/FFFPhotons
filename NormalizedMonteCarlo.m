...
%First step in normalization is taking the third derivative of the FF
%profile.

d1 = gradient(FFProfile);

d2 = gradient(d1);

d3 = gradient(d2);

%Pulling out the left side of the profile; this makes it easier to find the
%maxima of the third derivative
d3_left_profile = d3(1:zero_index);

[left_maximums, left_maxindex] = findpeaks(d3_left_profile);

left_max_matrix = zeros(length(left_maximums),2);

left_max_matrix(:,1) = left_maximums(:);

left_max_matrix(:,2) = left_maxindex(:);

left_max_matrix=sortrows(left_max_matrix,-1);

%if statement selects for the maximum value of the third derivative of the
%FF profile that is closest to the center 

if left_max_matrix(1,2) > left_max_matrix(2,2)
    
    maximum_index = left_max_matrix(1,2);
    
else 
    
    maximum_index = left_max_matrix(2,2);
    
end

NormValue = FFFProfile(maximum_index);

%Normalizing the FFF Profile
FFFNormalized = FFFProfile(:)/NormValue;
